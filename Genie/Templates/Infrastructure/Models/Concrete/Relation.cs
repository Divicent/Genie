...
");
        ...
        foreach(var key in _enum.Values) 
        {
            enm.AppendLine($@"		public static {_enum.Name} {key.Name} => {key.FieldName} ?? ( {key.FieldName} = new {_enum.Name}({key.Value}));");        
        }
        enm.AppendLine($@"	}}");
    }  
    ...
    {
        fkFields.AppendLine($@"		private {atd.ReferencingRelationName} {atd.ReferencingNonForeignKeyAttribute.FieldName}Obj;");
    }
