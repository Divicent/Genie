...
namespace {{baseNamespace}}.Infrastructure.Repositories
{
...
{% for relation in relations %}
    ...
    {% if relation.hasKeys %}
            ...
            {{relation.Name}} GetByKey({{relation.keyString}});
            ...
            {{relation.keyCommentString}}
            /// <returns>A registered {{relation.Name}} object</returns>
            Task<{{relation.Name}}> GetByKeyAsync({{relation.keyString}});
...
{% for relation in relations %}
    ...
    {% if relation.hasKeys %}
            public {{relation.Name}} GetByKey({{relation.keyString}})
            {
                return Get().Where{{relation.keyGetter}}.Filter().FirstOrDefault();
            }

            public async Task<{{relation.Name}}> GetByKeyAsync({{relation.keyString}})
            {
...
";
            ...
            return Process(nameof(RepositoryImplementationTemplate), template, new
            {
